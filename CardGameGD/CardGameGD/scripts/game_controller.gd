...
func create_players() -> void:
	...
	local_player.get_strength()[CardType.Type.FIRE] = 1
	local_player.get_strength()[CardType.Type.WATER] = 1
	local_player.get_strength()[CardType.Type.AIR] = 1
	local_player.get_strength()[CardType.Type.EARTH] = 1
	local_player.get_strength()[CardType.Type.OTHER] = 1
	...
	# Set starting resources
	opponent_player.get_strength()[CardType.Type.FIRE] = 1
	opponent_player.get_strength()[CardType.Type.WATER] = 1
	opponent_player.get_strength()[CardType.Type.AIR] = 1
	opponent_player.get_strength()[CardType.Type.EARTH] = 1
	opponent_player.get_strength()[CardType.Type.OTHER] = 1
...
func _add_turn_resources(player: Player) -> void:
	var strength := player.get_strength()
...
func _on_player_slot_clicked(slot: SlotVisual) -> void:
	...
	# Summon creature
	summon_creature(card, slot.get_index())
...
func _on_opponent_slot_clicked(slot: SlotVisual) -> void:
	...
	cast_spell(card, slot.get_index())
...
func summon_creature(card: Card, slot_index: int) -> void:
	...
	if SoundManager:
		SoundManager.play_sound_by_enum(SoundManager.Sound.SUMMON_DROP)
...
func _handle_remote_strength_change(event: NetworkEvent) -> void:
	...
	if type_affected >= 0 and type_affected < CardType.Type.size():
		opponent_player.get_strength()[type_affected] += strength_change
		opponent_visual.update_display()
...
func show_victory_defeat_screen(winner_id: String) -> void:
	...
	if is_victory:
		...
		# Play victory sound
		if SoundManager:
			SoundManager.play_sound_by_enum(SoundManager.Sound.GAMEOVER)
...
func show_card_tooltip(card: Card, mouse_pos: Vector2) -> void:
	...
	if card is Creature:
		...
		stats_label.text = "ATK: %d  HP: %d" % [creature.get_attack(), creature.get_life()]
