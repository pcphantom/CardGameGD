...
func _render_card(size_type: String) -> void:
	...
	if TextureManager and TextureManager.is_loaded:
		...
		# DEBUG: print("[CardImage._render_card] Rendering card texture for: ", card_name)
		...
		if card_texture != null:
			...
			print("[CardImage] ✓ Texture assigned for '%s': size=%s, class=%s" % [card_name, card_texture.get_size(), card_texture.get_class()])
		else:
			push_error("CardImage: ✗ MISSING TEXTURE for card: %s" % card_name)
...
func clone_card() -> CardImage:
	...
	if card != null:
		# DEBUG: print("[CardImage.clone_card] Cloning card: ", card.get_name())
		var cloned_card_data = card.clone()
		# DEBUG: print("[CardImage.clone_card] Cloned card name: ", cloned_card_data.get_name())
	...
	if creature != null:
		# DEBUG: print("[CardImage.clone_card] WARNING: Hand card has creature! This should not happen.")
